...
// New returns a Backend that shells out to the git binary to clone and materialise repositories.
func New() (Backend, error) {
	gitPath, err := exec.LookPath("git")
	if err != nil {
		return nil, fmt.Errorf("git binary not found: %w", err)
	}
	return &gitBinaryBackend{gitPath: gitPath}, nil
}

func (m *gitBinaryBackend) Materialize(ctx context.Context, attrs volume.Attributes, p policy.GitContentPolicy, workDir string) (Result, error) { //nolint:cyclop
	...
	if attrs.Submodules {
		if err := m.runGit(ctx, repoDir, "submodule", "update", "--init", "--depth", strconv.Itoa(p.Submodules.MaxDepth)); err != nil {
			return Result{}, fmt.Errorf("git submodule update failed: %w", err)
		}
	}

	resolved, err := m.gitOutput(ctx, repoDir, "rev-parse", "HEAD")
	if err != nil {
...
func (m *gitBinaryBackend) cloneOrRefresh(ctx context.Context, attrs volume.Attributes, _ policy.GitContentPolicy, workDir, repoDir string, depth int) error {
	...
	if isPinnedCommit(attrs.Revision) {
		...
		if err := m.runGit(ctx, workDir, "clone", "--no-checkout", "--depth", strconv.Itoa(depth), "--", attrs.Repo, repoDir); err != nil {
	...
	if err := m.runGit(ctx, workDir, args...); err != nil {
		return fmt.Errorf("git clone failed: %w", err)
	}
	return nil
}
...
func (m *gitBinaryBackend) fetchMutableRevision(ctx context.Context, attrs volume.Attributes, repoDir string) error {
	...
	if err := m.runGit(ctx, repoDir, fetchArgs...); err != nil {
...
func (m *gitBinaryBackend) checkoutRevision(ctx context.Context, attrs volume.Attributes, repoDir string) error {
	if isPinnedCommit(attrs.Revision) {
		if err := m.runGit(ctx, repoDir, "checkout", "--detach", attrs.Revision); err != nil {
	...
	sha, err := m.gitOutput(ctx, repoDir, "rev-parse", "--verify", "--end-of-options", ref+"^{commit}")
	if err != nil {
	...
	if err := m.runGit(ctx, repoDir, "checkout", "--detach", sha); err != nil {
		return fmt.Errorf("git checkout failed: %w", err)
	}
	return nil
}

func (m *gitBinaryBackend) runGit(ctx context.Context, dir string, args ...string) error {
	cmd, err := m.gitCommand(ctx, dir, args...)
	if err != nil {
		return err
	}
...
func (m *gitBinaryBackend) gitOutput(ctx context.Context, dir string, args ...string) (string, error) {
	cmd, err := m.gitCommand(ctx, dir, args...)
	if err != nil {
		return "", err
	}
...
func (m *gitBinaryBackend) gitCommand(ctx context.Context, dir string, args ...string) (*exec.Cmd, error) {
	if err := validateGitArgs(args); err != nil {
		return nil, err
	}
	cmd := exec.CommandContext(ctx, m.gitPath, gitArgs(args...)...) // #nosec G204 -- m.gitPath is an absolute path resolved by exec.LookPath; args are validated before reaching here.
	cmd.Dir = dir
...
// CopyContentTree copies src into dst, skipping .git directories and rejecting symlinks that escape src.
